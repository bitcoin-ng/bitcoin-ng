...
print(f"DEBUG: MAGIC_BYTES['mainnet'] = {MAGIC_BYTES['mainnet'].hex()}")
...
def initialize_v2_transport(ecdh_secret, initiating, net="mainnet"):
    peer = {}
    salt = b'bitcoin_v2_shared_secret' + MAGIC_BYTES[net]
    ...
    if initiating:
        ...
        peer['send_garbage_terminator'] = peer['garbage_terminators'][:16]
        peer['recv_L'] = FSChaCha20(peer['responder_L'])
        peer['recv_P'] = FSChaCha20Poly1305(peer['responder_P'])
        peer['recv_garbage_terminator'] = peer['garbage_terminators'][16:]
    else:
...
def v2_enc_packet(peer, contents, aad=b'', ignore=False):
    header = (int(ignore) << IGNORE_BIT_POS).to_bytes(HEADER_LEN, 'little')
...
def generate_vector(in_idx, in_priv_ours_hex, in_ellswift_ours_hex, in_ellswift_theirs_hex, in_initiating, in_contents_hex, in_multiply, in_aad_hex, in_ignore):
    priv_ours = bytes.fromhex(in_priv_ours_hex)
    ...
    ecdh_secret = v2_ecdh(priv_ours, ellswift_theirs, ellswift_ours, in_initiating)
    peer = initialize_v2_transport(ecdh_secret, in_initiating, net="mainnet")
    
    # Encrypt dummy packets to advance state
    for _ in range(in_idx):
        v2_enc_packet(peer, b'', aad=b'', ignore=True)
        
    ciphertext = v2_enc_packet(peer, contents, aad=aad, ignore=in_ignore)
    
    print(f"    TestBIP324PacketVector(")
    print(f"        {in_idx},")
    print(f"        \"{in_priv_ours_hex}\",")
    print(f"        \"{in_ellswift_ours_hex}\",")
    print(f"        \"{in_ellswift_theirs_hex}\",")
    print(f"        {'true' if in_initiating else 'false'},")
    print(f"        \"{in_contents_hex}\",")
    print(f"        {in_multiply},")
    print(f"        \"{in_aad_hex}\",")
    print(f"        {'true' if in_ignore else 'false'},")
    print(f"        \"{peer['send_garbage_terminator'].hex()}\",")
    print(f"        \"{peer['recv_garbage_terminator'].hex()}\",")
    print(f"        \"{peer['session_id'].hex()}\",")
    if len(ciphertext) < 200: # Arbitrary limit to switch to endswith
        print(f"        \"{ciphertext.hex()}\",")
        print(f"        \"\");")
    else:
        print(f"        \"\",")
        print(f"        \"{ciphertext[-128:].hex()}\");") # Use last 128 bytes (256 hex chars)

# Test vectors from bip324_tests.cpp (inputs only)
vectors = [
