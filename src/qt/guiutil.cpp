...
{
    // return if URI is not valid or is no bng: URI
    if(!uri.isValid() || uri.scheme() != QString("bng"))
        return false;
...
QString formatBitcoinURI(const SendCoinsRecipient &info)
{
    ...
    QString ret = QString("bng:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;
