...
using bng::test::FixupAddrDescriptorForExpectedOutput;
using bng::test::FixupDescriptorForParsing;
using bng::test::ReencodeWifKeys;
using bng::test::ReencodeWifKeysPreserveChecksum;
using bng::test::StripChecksumIfPresent;

void CheckUnparsable(const std::string& prv, const std::string& pub, const std::string& expected_error)
{
    const std::string prv_fixed = ReencodeWifKeys(prv);
    const std::string pub_fixed = ReencodeWifKeys(pub);
    const std::string expected_error_fixed = ReencodeWifKeys(expected_error);

    FlatSigningProvider keys_priv, keys_pub;
    std::string error;
    auto parse_priv = Parse(prv_fixed, keys_priv, error);
    auto parse_pub = Parse(pub_fixed, keys_pub, error);
    BOOST_CHECK_MESSAGE(parse_priv.empty(), prv_fixed);
    BOOST_CHECK_MESSAGE(parse_pub.empty(), pub_fixed);
    BOOST_CHECK_EQUAL(error, expected_error_fixed);
}
...
static bool IsExtPubKeyToken(const std::string& token)
{
    std::vector<unsigned char> data;
    if (!DecodeBase58Check(token, data, /*max_ret_len=*/100)) return false;
    if (data.size() != BIP32_EXTKEY_WITH_VERSION_SIZE) return false;
    ...
    return keydata_prefix == 0x02 || keydata_prefix == 0x03;
}

// Count the number of extended pubkeys in a descriptor string
static size_t CountXpubs(const std::string& desc)
{
    ...
    for (auto it = std::sregex_iterator{desc.begin(), desc.end(), base58_re}; it != std::sregex_iterator{}; ++it) {
        if (IsExtPubKeyToken(it->str())) ++count;
    }
    return count;
}
//
// Count the number of unique extended pubkeys in a descriptor string
static size_t CountUniqueXpubs(const std::string& desc)
{
    std::set<std::string> extpubs;
    static const std::regex base58_re{"[123456789ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz]{20,}"};
    for (auto it = std::sregex_iterator{desc.begin(), desc.end(), base58_re}; it != std::sregex_iterator{}; ++it) {
        const std::string token = it->str();
        if (IsExtPubKeyToken(token)) extpubs.emplace(token);
    }
    return extpubs.size();
}
...
{
    prv = FixupDescriptorForParsing(std::move(prv));
    pub = FixupDescriptorForParsing(std::move(pub));
    const std::string norm_pub_fixed = StripChecksumIfPresent(ReencodeWifKeysPreserveChecksum(norm_pub));
    if (expected_prv) expected_prv = ReencodeWifKeysPreserveChecksum(*expected_prv);
    if (expected_pub) expected_pub = ReencodeWifKeysPreserveChecksum(*expected_pub);
    ...
    BOOST_CHECK_MESSAGE(EqualDescriptor(norm1, norm_pub_fixed), "priv->ToNormalizedString(): " + norm1 + " Norm. desc: " + norm_pub_fixed);
    BOOST_CHECK(parse_pub->ToNormalizedString(keys_priv, norm1));
    BOOST_CHECK_MESSAGE(EqualDescriptor(norm1, norm_pub_fixed), "pub->ToNormalizedString(): " + norm1 + " Norm. desc: " + norm_pub_fixed);
        ...
        const std::vector<std::set<std::vector<uint32_t>>>& paths)
{
    ...
    std::string error;
    const auto desc{[&](){
        const std::string pub_fixed = FixupDescriptorForParsing(pub);
        auto parsed{Parse(pub_fixed, prov, error)};
...
void CheckInferDescriptor(const std::string& script_hex, const std::string& expected_desc, const std::vector<std::string>& hex_scripts, const std::vector<std::pair<std::string, std::string>>& origin_pubkeys)
{
    ...
    const std::string expected_desc_fixed = FixupAddrDescriptorForExpectedOutput(expected_desc);
    std::string checksum{GetDescriptorChecksum(expected_desc_fixed)};

    std::unique_ptr<Descriptor> desc = InferDescriptor(script, provider);
    BOOST_CHECK_EQUAL(desc->ToString(), expected_desc_fixed + "#" + checksum);
}
...
BOOST_AUTO_TEST_CASE(descriptor_test_1)
{
    // NOTE (BNG): This file is being brought up in small chunks.
    // Keep enabling the next test case (descriptor_test_N) and fix failures
    // (often DescriptorID changes due to key re-encoding) before proceeding.

    // Chunk 1: Basic single-key compressed
    ...
    // Chunk 2: Key origin parse errors + basic single-key uncompressed
...
BOOST_AUTO_TEST_CASE(descriptor_test_2)
{
    // Chunk 5: Multisig constructions + checksums + addr/raw + rawtr
    ...
    Check(
        ...
        OutputType::BECH32M,
        /*op_desc_id=*/uint256{"227da5b5874844d5e7ec0b359286fefdf592133a396c874b656bb0db4342444b"},
...
BOOST_AUTO_TEST_CASE(descriptor_test_3)
{
    // Chunk 6: Miniscript tests
...
BOOST_AUTO_TEST_CASE(descriptor_test_4)
{
    // Chunk 7: Descriptor inference + MuSig2 parsing
