...
public:
     ...
     * and decryption can be tested without knowing the other side's private key.
        * message_start_override is only for testing, and when set overrides the message start bytes
        * used in the HKDF salt instead of using Params().MessageStart().
     */
        void Initialize(const EllSwiftPubKey& their_pubkey, bool initiator, bool self_decrypt = false,
                    std::optional<MessageStartChars> message_start_override = std::nullopt) noexcept;
