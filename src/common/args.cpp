...
fs::path GetDefaultDataDir()
{
    ...
    fs::path legacy_path = GetSpecialFolderPath(CSIDL_APPDATA) / "BitcoinNG";
    if (fs::exists(legacy_path)) return legacy_path;

    // Otherwise, fresh installs can start in the new, "proper" location
    return GetSpecialFolderPath(CSIDL_LOCAL_APPDATA) / "BitcoinNG";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/BitcoinNG";
#else
    // Unix-like
    return pathRet / ".bitcoin-ng";
#endif
#endif
}
